...
{
    private readonly PitScoutingViewModel _viewModel;

    public PitScoutingPage(PitScoutingViewModel viewModel)
    {
 _viewModel = viewModel;
        BindingContext = viewModel;
        InitializeComponent();

        // Subscribe to property changes to rebuild form when config loads
        _viewModel.PropertyChanged += ViewModel_PropertyChanged;
    }

    private void ViewModel_PropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(PitScoutingViewModel.PitConfig))
      {
 BuildDynamicForm();
        }
    }
    ...
    {
        if (_viewModel.PitConfig?.PitScouting?.Sections == null)
      {
System.Diagnostics.Debug.WriteLine("[PitScouting] BuildDynamicForm: No sections available");
            return;
        }

        System.Diagnostics.Debug.WriteLine($"[PitScouting] BuildDynamicForm: Building form with {_viewModel.PitConfig.PitScouting.Sections.Count} sections");

        FormContainer.Children.Clear();

        foreach (var section in _viewModel.PitConfig.PitScouting.Sections)
        {
    System.Diagnostics.Debug.WriteLine($"[PitScouting] Section: {section.Name} with {section.Elements.Count} elements");
...
Placeholder = element.Placeholder ?? $"Enter {element.Name}",
 ...
 _viewModel.SetFieldValue(element.Id, e.Value);
    ...
    {
        System.Diagnostics.Debug.WriteLine($"[PitScouting] CreatePicker for {element.Name}: Options={element.Options?.Count ?? 0}");
    
        if (element.Options == null)
        {
      System.Diagnostics.Debug.WriteLine($"[PitScouting] WARNING: Options is NULL for {element.Name}");
            return new Picker 
 { 
       ...
       FontSize = 16,
        ...
        {
  System.Diagnostics.Debug.WriteLine($"[PitScouting] WARNING: Options is EMPTY for {element.Name}");
       return new Picker 
       { 
     Title = $"No options configured for {element.Name}", 
   IsEnabled = false,
 ...
 {
        System.Diagnostics.Debug.WriteLine($"[PitScouting]   Adding option: {option.Label} (value: {option.Value})");
  picker.Items.Add(option.Label);
        }

        System.Diagnostics.Debug.WriteLine($"[PitScouting] Picker for {element.Name} now has {picker.Items.Count} items");
 ...
 picker.SelectedIndex = index;
  ...
  return picker;
    ...
    {
        System.Diagnostics.Debug.WriteLine($"[PitScouting] CreateMultiSelectPicker for {element.Name}: Options={element.Options?.Count ?? 0}");
        
        var container = new VerticalStackLayout { Spacing = 8 };

        if (element.Options == null || element.Options.Count == 0)
        {
 System.Diagnostics.Debug.WriteLine($"[PitScouting] WARNING: No options for multiselect {element.Name}");
  container.Add(new Label 
   ...
   foreach (var option in element.Options)
        {
  System.Diagnostics.Debug.WriteLine($"[PitScouting]   Adding multiselect option: {option.Label}");
