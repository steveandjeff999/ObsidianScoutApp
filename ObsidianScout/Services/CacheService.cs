...
public class CacheService : ICacheService
{
    ...
    /// Data persists across app restarts via SecureStorage (uses platform-specific secure storage)
    /// </summary>
    ...
    public async Task<bool> HasCachedDataAsync()
    {
        try
        {
            ...
            var gameConfig = await SecureStorage.GetAsync(CACHE_KEY_GAME_CONFIG);
            var events = await SecureStorage.GetAsync(CACHE_KEY_EVENTS);
    ...
    public async Task<List<Event>?> GetCachedEventsAsync()
    {
        try
        {
            var json = await SecureStorage.GetAsync(CACHE_KEY_EVENTS);
            if (!string.IsNullOrEmpty(json))
            {
    ...
    public async Task CacheEventsAsync(List<Event> events)
    {
        try
        {
            ...
            await SecureStorage.SetAsync(CACHE_KEY_EVENTS, json);
    ...
    public async Task<List<MetricDefinition>?> GetCachedAvailableMetricsAsync()
    {
        try
        {
            var json = await SecureStorage.GetAsync(CACHE_KEY_AVAILABLE_METRICS);
            if (!string.IsNullOrEmpty(json))
            {
    ...
    public async Task CacheAvailableMetricsAsync(List<MetricDefinition> metrics)
    {
        try
        {
            ...
            await SecureStorage.SetAsync(CACHE_KEY_AVAILABLE_METRICS, json);
    ...
    public async Task<List<ScoutingEntry>?> GetCachedScoutingDataAsync()
    {
        try
        {
            var json = await SecureStorage.GetAsync(CACHE_KEY_SCOUTING_DATA);
            if (!string.IsNullOrEmpty(json))
            {
    ...
    public async Task CacheScoutingDataAsync(List<ScoutingEntry> scoutingData)
    {
        try
        {
            ...
            await SecureStorage.SetAsync(CACHE_KEY_SCOUTING_DATA, json);
    ...
    public async Task<DateTime?> GetCacheTimestampAsync(string key)
    {
        try
        {
            var timestampStr = await SecureStorage.GetAsync($"{key}{TIMESTAMP_SUFFIX}");
    ...
    private async Task SetCacheTimestampAsync(string key)
    {
        try
        {
            await SecureStorage.SetAsync($"{key}{TIMESTAMP_SUFFIX}", DateTime.UtcNow.ToString("O"));
        }
        catch (Exception ex)
        {
    ...
    public async Task ClearAllCacheAsync()
    {
        try
        {
            ...
            foreach (var key in cacheKeys)
            {
                ...
                SecureStorage.Remove($"{key}{TIMESTAMP_SUFFIX}");
            }
