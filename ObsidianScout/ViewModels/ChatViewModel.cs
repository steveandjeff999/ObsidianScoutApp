...
public partial class ChatViewModel : ObservableObject
{
 ...
 try { _ = LoadMessagesCommand.ExecuteAsync(null); } catch { }
 ...
 try
 {
 // only poll if a recipient is selected
 if (SelectedMember == null) return;

 var resp = await _apiService.GetChatMessagesAsync(type: ChatType, user: SelectedMember.Id.ToString());
 ...
 {
 var resp = await _apiService.GetChatMessagesAsync(ChatType, ChatUser, ChatGroup);
 Messages.Clear();
 if (resp.Success && resp.Messages != null)
 {
 var sorted = resp.Messages.OrderBy(m => m.Timestamp).ToList();
 ...
 // If sending as alliance/group, set conversation type
 if (!string.IsNullOrEmpty(ChatGroup))
 {
 req.ConversationType = ChatGroup;
 }
