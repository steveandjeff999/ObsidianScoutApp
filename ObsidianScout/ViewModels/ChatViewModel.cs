...
public partial class ChatViewModel : ObservableObject
{
 ...
 // Async lock to prevent concurrent operations that mutate shared state
 private readonly AsyncLock _vmLock = new();
...
ChatUser = value?.Id.ToString();
 ...
 {
 // Prevent concurrent polls / loads
 using (await _vmLock.LockAsync())
 {
 try
 {
 ...
 {
 // Batch UI update on main thread
 await Microsoft.Maui.ApplicationModel.MainThread.InvokeOnMainThreadAsync(() =>
 {
 // Only update the most recent messages, preserve older loaded ones
 var oldLast = LastMessageTimestamp;
 // Remove and re-add recent messages that are within the polled window
 var recentMessages = _messages.Where(m => oldLast == null ? false : m.Timestamp > oldLast).ToList();
 ...
 var cutoff = LastMessageTimestamp == null ? DateTime.MinValue : LastMessageTimestamp.Value.AddMinutes(-5);

 foreach (var local in _messages.Where(m => m.Timestamp >= cutoff).ToList())
 {
 ...
 if (!append)
 {
 await Microsoft.Maui.ApplicationModel.MainThread.InvokeOnMainThreadAsync(() => Messages.Clear());
 ...
 {
 // keep UI state toggles but guard operation
 try
 {
 IsLoading = true;
 ...
 // Do not call LoadMessagesAsync here to avoid reentrancy with polling; rely on poll or manual refresh
 }
 else
 {
